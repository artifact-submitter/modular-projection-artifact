...
import CertifiedJL.Analysis.Gaussian.HalfGaussian512
import CertifiedJL.Projection.Counterexamples.L2Upper.EvenGaussianComparison

/-!
# Compatibility facade for the 512-row Gaussian comparison

The proof is row-parametric in `EvenGaussianComparison`; this module retains
the established 512-row names used by the two concrete endpoint assemblies.
-/
...
open Probability
namespace Counterexamples.SparseUpper.Rows512.Internal
...
noncomputable def radialShift : ℝ := 512 * mesh * coordinateBound
...
noncomputable def totalLoss : ℝ := radialShift + 512 * localLoss
...
noncomputable abbrev boundedVectors : Finset (Fin 512 → ℤ) :=
  EvenGaussian.Internal.boundedVectors
abbrev discreteSqNorm (z : Fin 512 → ℤ) : ℕ := EvenGaussian.Internal.discreteSqNorm z
noncomputable abbrev goodVectors (threshold : ℕ) : Finset (Fin 512 → ℤ) :=
  EvenGaussian.Internal.goodVectors threshold
abbrev gaussianBox (z : Fin 512 → ℤ) : Set (Fin 512 → ℝ) :=
  EvenGaussian.Internal.gaussianBox z
noncomputable abbrev roundIndex (x : ℝ) : ℤ := EvenGaussian.Internal.roundIndex x
noncomputable abbrev roundVector (x : Fin 512 → ℝ) : Fin 512 → ℤ :=
  EvenGaussian.Internal.roundVector x
abbrev gaussianCore (threshold : ℕ) : Set (Fin 512 → ℝ) :=
...
noncomputable abbrev sparseAllOnesRowSumsPMF : PMF (Fin 512 → ℤ) :=
...
theorem HalfGaussian512.halfGaussianDensity_le_cellCenter {k : ℤ} {x : ℝ}
    (hk : k.natAbs ≤ cutoff) (hx : x ∈ gaussianCell k) :
    HalfGaussian512.halfGaussianDensity x ≤
      HalfGaussian512.halfGaussianDensity ((k : ℝ) * mesh) *
        Real.exp (10 / side : ℝ) := by
  simpa [HalfGaussian512.halfGaussianDensity,
...
abbrev mem_goodVectors_iff {threshold : ℕ} {z : Fin 512 → ℤ} :=
  EvenGaussian.Internal.mem_goodVectors_iff
    (rows := 512) (threshold := threshold) (z := z)
abbrev measurableSet_gaussianBox (z : Fin 512 → ℤ) :=
  EvenGaussian.Internal.measurableSet_gaussianBox z
abbrev gaussianBox_disjoint {z z' : Fin 512 → ℤ} (hzz' : z ≠ z') :=
  EvenGaussian.Internal.gaussianBox_disjoint hzz'
abbrev integral_gaussianBox (z : Fin 512 → ℤ) :=
...
abbrev mem_gaussianBox_roundVector (x : Fin 512 → ℝ) :=
...
abbrev measurableSet_gaussianCore (threshold : ℕ) :=
  EvenGaussian.Internal.measurableSet_gaussianCore
    (rows := 512) threshold
abbrev roundVector_mem_boundedVectors {threshold : ℕ} {x : Fin 512 → ℝ}
    (hx : x ∈ gaussianCore threshold) :=
  EvenGaussian.Internal.roundVector_mem_boundedVectors hx
abbrev roundVector_sqNorm_gt {threshold : ℕ} {x : Fin 512 → ℝ}
    (hx : x ∈ gaussianCore threshold) :=
  EvenGaussian.Internal.roundVector_sqNorm_gt hx
abbrev roundVector_mem_goodVectors {threshold : ℕ} {x : Fin 512 → ℝ}
    (hx : x ∈ gaussianCore threshold) :=
...
abbrev gaussianCore_subset_goodBoxes (threshold : ℕ) :=
  EvenGaussian.Internal.gaussianCore_subset_goodBoxes
    (rows := 512) threshold
abbrev mem_boundedVectors_index {z : Fin 512 → ℤ}
    (hz : z ∈ boundedVectors) (j : Fin 512) :=
...
abbrev integral_gaussianCore_le_boxSum (threshold : ℕ) :=
  EvenGaussian.Internal.integral_gaussianCore_le_boxSum
    (rows := 512) threshold
abbrev sparseAllOnesRowSumsPMF_apply (z : Fin 512 → ℤ) :=
  EvenGaussian.Internal.sparseAllOnesRowSumsPMF_apply z
abbrev goodVector_pointMass_ge {threshold : ℕ} {z : Fin 512 → ℤ}
...
theorem sparseAllOnesRowSumsPMF_eq_map_uniformSeed :
    sparseAllOnesRowSumsPMF =
      (PMF.uniformOfFintype (SparseSeed 512 dimension)).map
...
theorem tensorComparison (threshold : ℕ) :
    ENNReal.ofReal
        (Real.exp (-(512 : ℝ) * localLoss) *
          ∫ x : Fin 512 → ℝ in gaussianCore threshold,
            HalfGaussian512.halfGaussianProductDensity x) ≤
      sparseAllOnesRowSumsPMF.toMeasure
        {z | threshold * dimension < discreteSqNorm z} := by
  simpa [HalfGaussian512.halfGaussianProductDensity,
    HalfGaussian512.halfGaussianDensity,
    ...
    (EvenGaussian.Internal.tensorComparison (rows := 512) threshold)

end Counterexamples.SparseUpper.Rows512.Internal
end CertifiedJL
