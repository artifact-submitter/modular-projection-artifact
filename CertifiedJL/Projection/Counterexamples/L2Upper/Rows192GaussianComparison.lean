...
import CertifiedJL.Analysis.Gaussian.HalfGaussian192
import CertifiedJL.Projection.Counterexamples.L2Upper.EvenGaussianComparison

/-!
# Compatibility facade for the 192-row Gaussian comparison

The proof is row-parametric in `EvenGaussianComparison`; this module retains
the established 192-row names used by the two concrete endpoint assemblies.
-/
...
abbrev side : ℕ := rows192L2UpperCounterexampleSide
abbrev dimension : ℕ := rows192L2UpperCounterexampleDimension
def cutoff : ℕ := 10 * side
noncomputable def mesh : ℝ := (side : ℝ)⁻¹
noncomputable def coordinateBound : ℝ := 10 - mesh / 2
noncomputable def radialShift : ℝ := 192 * mesh * coordinateBound
...
noncomputable def totalLoss : ℝ := radialShift + 192 * localLoss

@[simp] theorem side_eq : side = 100_000_000 := rfl
@[simp] theorem dimension_eq : dimension = 10_000_000_000_000_000 := by
  norm_num [dimension, side, rows192L2UpperCounterexampleDimension,
...
noncomputable abbrev boundedVectors : Finset (Fin 192 → ℤ) :=
  EvenGaussian.Internal.boundedVectors
abbrev discreteSqNorm (z : Fin 192 → ℤ) : ℕ := EvenGaussian.Internal.discreteSqNorm z
noncomputable abbrev goodVectors (threshold : ℕ) : Finset (Fin 192 → ℤ) :=
  EvenGaussian.Internal.goodVectors threshold
abbrev gaussianBox (z : Fin 192 → ℤ) : Set (Fin 192 → ℝ) :=
  EvenGaussian.Internal.gaussianBox z
noncomputable abbrev roundIndex (x : ℝ) : ℤ := EvenGaussian.Internal.roundIndex x
noncomputable abbrev roundVector (x : Fin 192 → ℝ) : Fin 192 → ℤ :=
  EvenGaussian.Internal.roundVector x
abbrev gaussianCore (threshold : ℕ) : Set (Fin 192 → ℝ) :=
...
noncomputable abbrev sparseAllOnesRowSumsPMF : PMF (Fin 192 → ℤ) :=
...
theorem HalfGaussian192.halfGaussianDensity_le_cellCenter {k : ℤ} {x : ℝ}
    (hk : k.natAbs ≤ cutoff) (hx : x ∈ gaussianCell k) :
    HalfGaussian192.halfGaussianDensity x ≤
      HalfGaussian192.halfGaussianDensity ((k : ℝ) * mesh) *
        Real.exp (10 / side : ℝ) := by
  simpa [HalfGaussian192.halfGaussianDensity,
...
abbrev mem_goodVectors_iff {threshold : ℕ} {z : Fin 192 → ℤ} :=
  EvenGaussian.Internal.mem_goodVectors_iff
    (rows := 192) (threshold := threshold) (z := z)
abbrev measurableSet_gaussianBox (z : Fin 192 → ℤ) :=
  EvenGaussian.Internal.measurableSet_gaussianBox z
abbrev gaussianBox_disjoint {z z' : Fin 192 → ℤ} (hzz' : z ≠ z') :=
  EvenGaussian.Internal.gaussianBox_disjoint hzz'
abbrev integral_gaussianBox (z : Fin 192 → ℤ) :=
...
abbrev mem_gaussianBox_roundVector (x : Fin 192 → ℝ) :=
...
abbrev measurableSet_gaussianCore (threshold : ℕ) :=
  EvenGaussian.Internal.measurableSet_gaussianCore
    (rows := 192) threshold
abbrev roundVector_mem_boundedVectors {threshold : ℕ} {x : Fin 192 → ℝ}
    (hx : x ∈ gaussianCore threshold) :=
  EvenGaussian.Internal.roundVector_mem_boundedVectors hx
abbrev roundVector_sqNorm_gt {threshold : ℕ} {x : Fin 192 → ℝ}
    (hx : x ∈ gaussianCore threshold) :=
  EvenGaussian.Internal.roundVector_sqNorm_gt hx
abbrev roundVector_mem_goodVectors {threshold : ℕ} {x : Fin 192 → ℝ}
    (hx : x ∈ gaussianCore threshold) :=
...
abbrev gaussianCore_subset_goodBoxes (threshold : ℕ) :=
  EvenGaussian.Internal.gaussianCore_subset_goodBoxes
    (rows := 192) threshold
abbrev mem_boundedVectors_index {z : Fin 192 → ℤ}
    (hz : z ∈ boundedVectors) (j : Fin 192) :=
...
abbrev integral_gaussianCore_le_boxSum (threshold : ℕ) :=
  EvenGaussian.Internal.integral_gaussianCore_le_boxSum
    (rows := 192) threshold
abbrev sparseAllOnesRowSumsPMF_apply (z : Fin 192 → ℤ) :=
  EvenGaussian.Internal.sparseAllOnesRowSumsPMF_apply z
abbrev goodVector_pointMass_ge {threshold : ℕ} {z : Fin 192 → ℤ}
...
theorem sparseAllOnesRowSumsPMF_eq_map_uniformSeed :
    sparseAllOnesRowSumsPMF =
      (PMF.uniformOfFintype (SparseSeed 192 dimension)).map
...
theorem tensorComparison (threshold : ℕ) :
    ENNReal.ofReal
        (Real.exp (-(192 : ℝ) * localLoss) *
          ∫ x : Fin 192 → ℝ in gaussianCore threshold,
            HalfGaussian192.halfGaussianProductDensity x) ≤
      sparseAllOnesRowSumsPMF.toMeasure
        {z | threshold * dimension < discreteSqNorm z} := by
  simpa [HalfGaussian192.halfGaussianProductDensity,
    HalfGaussian192.halfGaussianDensity,
    ...
    (EvenGaussian.Internal.tensorComparison (rows := 192) threshold)
