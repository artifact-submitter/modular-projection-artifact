...
/-!
# Sharp two-decimal `17/50` sparse modular infinity lower tail
-/

open scoped BigOperators ENNReal
open MeasureTheory

namespace CertifiedJL.TernaryLInfThresholdLower17Over50TwoDecimal
...
def Small (q : ℕ) {d : ℕ} (w : Fin d → ℤ) : Prop :=
  250 ^ 2 * sqNorm w < 111 ^ 2 * q ^ 2

def Large (q : ℕ) {d : ℕ} (w : Fin d → ℤ) : Prop :=
  ∃ i, 17 * q < 50 * (w i).natAbs

def Diffuse (q : ℕ) {d : ℕ} (w : Fin d → ℤ) : Prop :=
  111 ^ 2 * q ^ 2 ≤ 250 ^ 2 * sqNorm w ∧
    ∀ i, 50 * (w i).natAbs ≤ 17 * q

def CentralRow (inputThreshold : ℕ) {d : ℕ} (w : Fin d → ℤ)
    (row : Fin d → ℤ) : Prop :=
  2500 * (∑ i, row i * w i).natAbs ^ 2 ≤ 289 * inputThreshold ^ 2

def WrapRow (q inputThreshold : ℕ) {d : ℕ} (w : Fin d → ℤ)
    (row : Fin d → ℤ) : Prop :=
  50 * q ≤ 17 * inputThreshold + 50 * (∑ i, row i * w i).natAbs

def DiffuseArcRow (q : ℕ) {d : ℕ} (w : Fin d → ℤ)
    (row : Fin d → ℤ) : Prop :=
  100 * (centeredMod q (∑ i, row i * w i)).natAbs ≤ 17 * q

theorem regime_trichotomy (q : ℕ) {d : ℕ} (w : Fin d → ℤ) :
    Small q w ∨ Large q w ∨ Diffuse q w := by
  by_cases hs : 250 ^ 2 * sqNorm w < 111 ^ 2 * q ^ 2
  · exact Or.inl hs
  · right
    by_cases hl : ∃ i, 17 * q < 50 * (w i).natAbs
    · exact Or.inl hl
...
theorem rowPass_implies_central_or_wrap
  ...
  · left
    ...
    change 2500 * z.natAbs ^ 2 ≤ 289 * inputThreshold ^ 2
    change 2500 * (centeredMod q z).natAbs ^ 2 ≤
      289 * inputThreshold ^ 2 at hp
    rwa [← hz] at hp
  · right
    have hgeometry := centeredMod_modulus_le_natAbs_add_of_ne hz
    have hr : 50 * (centeredMod q z).natAbs ≤ 17 * inputThreshold := by
      unfold RowPass at hp
      change 2500 * (centeredMod q z).natAbs ^ 2 ≤
        289 * inputThreshold ^ 2 at hp
      nlinarith [sq_nonneg
        (50 * ((centeredMod q z).natAbs : ℤ) - 17 * (inputThreshold : ℤ))]
    unfold WrapRow
    change q ≤ z.natAbs + (centeredMod q z).natAbs at hgeometry
    change 50 * q ≤ 17 * inputThreshold + 50 * z.natAbs
    omega

theorem central_row_probability_toReal_le {d inputThreshold : ℕ}
    (hcentral : TernaryLInfTwoDecimal.Cap34CentralCertificate)
    ...
    (eventProbability (sparseRademacherRow d)
      (CentralRow inputThreshold w)).toReal ≤
        (TernaryLInfTwoDecimal.Cap34Central.expectationBound : ℝ) := by
  classical
  ...
  apply central_event_probability_toReal_le w hw (CentralRow inputThreshold w)
    TernaryLInfTwoDecimal.Cap34Central.fourier
  ...
  · rw [if_pos hr]
    ...
    have hrR : (50 : ℝ) * |((∑ i, row i * w i : ℤ) : ℝ)| ≤
        17 * inputThreshold := by
      unfold CentralRow at hr
      have hcast : (2500 : ℝ) * |((∑ i, row i * w i : ℤ) : ℝ)| ^ 2 ≤
          289 * (inputThreshold : ℝ) ^ 2 := by
        have hcastNat :
            ((2500 * (∑ i, row i * w i).natAbs ^ 2 : ℕ) : ℝ) ≤
              ((289 * inputThreshold ^ 2 : ℕ) : ℝ) := by
          exact_mod_cast hr
    ...
    have hphase : |centralPhase w row| ≤
        (111 / 100 : ℝ) * (17 / 50 : ℝ) * Real.sqrt 2 := by
    ...
    have hcapPi : ((111 / 100 : ℝ) * (17 / 50 : ℝ) * Real.sqrt 2) ≤
        Real.pi := by
    ...
    have hc := cos_central34_lower
    norm_num at hc ⊢
    rw [show (111 / 100 : ℝ) * (17 / 50 : ℝ) = 1887 / 5000 by norm_num]
      at hcosMono
...
theorem diffuseArc_row_probability_toReal_lt {q d : ℕ}
    (hdiffuse : TernaryLInfTwoDecimal.Cap34DiffuseCertificate)
  ...
  have hpoint (row : Fin d → ℤ) :
      (if DiffuseArcRow q w row then (1 : ℝ) else 0) ≤
        TrigonometricBernstein.rationalCosineValue
          TernaryLInfTwoDecimal.Cap34Diffuse.fourier
          (diffusePhase (q := q) w row) := by
    by_cases hp : DiffuseArcRow q w row
    · rw [if_pos hp]
      let z : ℤ := ∑ i, row i * w i
      let r : ℤ := centeredMod q z
      have hr : (100 : ℝ) * |(r : ℝ)| ≤ 17 * q := by
        have hc : 100 * r.natAbs ≤ 17 * q := hp
        have hc' : ((100 * r.natAbs : ℕ) : ℝ) ≤ ((17 * q : ℕ) : ℝ) := by
          exact_mod_cast hc
      ...
      have hcapPi : (17 * Real.pi / 50 : ℝ) ≤ Real.pi := by
        nlinarith [Real.pi_pos]
      have hcosMono : Real.cos (17 * Real.pi / 50) ≤
          Real.cos ((2 * Real.pi / (q : ℝ)) * (r : ℝ)) := by
        calc
      ...
      rw [hcosPhase]
      have hc := cos_diffuse34_lower
      norm_num at hc ⊢
      rw [show 17 * Real.pi / 50 = Real.pi * (17 / 50 : ℝ) by ring]
        at hcosMono
      exact hc.le.trans hcosMono
    · rw [if_neg hp]
  ...
  have hmomentOne :
      (∫ seed, Real.cos ((1 : ℝ) *
        diffusePhase (q := q) w (sparseRow seed)) ∂p.toMeasure) ≤
          (TernaryLInfTwoDecimal.Cap34Diffuse.diffuseMomentBound : ℝ) := by
    ...
    have hsum : Real.pi ^ 2 * (111 / 250 : ℝ) ^ 2 ≤
        ∑ i, (Real.pi * (w i : ℝ) / (q : ℝ)) ^ 2 := by
      have hdR : (111 : ℝ) ^ 2 * q ^ 2 ≤
          (250 : ℝ) ^ 2 * (sqNorm w : ℝ) := by exact_mod_cast hd.1
    ...
    have hstrict : (∏ i, Real.cos
        (Real.pi * (w i : ℝ) / (q : ℝ)) ^ 2) < 71447 / 500000 := calc
      ...
      _ ≤ Real.exp (-(Real.pi ^ 2 * (111 / 250 : ℝ) ^ 2)) :=
        Real.exp_le_exp.mpr (by linarith)
      _ < 71447 / 500000 := exp_neg_diffuse34_lt
    norm_num [TernaryLInfTwoDecimal.Cap34Diffuse.diffuseMomentBound]
      at hstrict ⊢
    exact hstrict.le
  ...
  exact hbound.trans_lt (by
    norm_num [TernaryLInfTwoDecimal.Cap34Diffuse.expectationBound])

theorem wrap_implies_tail_36567_over_5000_lt
  ...
  have hbNorm : (inputThreshold : ℝ) ≤ ‖v‖ := by
    ...
    rw [← hnormSq] at hnormR
    nlinarith [norm_nonneg v]
  have hsR : (250 : ℝ) ^ 2 * (sqNorm w : ℝ) <
      (111 : ℝ) ^ 2 * q ^ 2 := by
    exact_mod_cast hs
  have hqNorm : (250 : ℝ) * ‖v‖ < 111 * q := by
    rw [← hnormSq] at hsR
    nlinarith [norm_nonneg v]
  have hrR : (50 : ℝ) * q ≤ 17 * inputThreshold +
      50 * |((∑ i, row i * w i : ℤ) : ℝ)| := by
    unfold WrapRow at hr
    have hcast : ((50 * q : ℕ) : ℝ) ≤
        ((17 * inputThreshold + 50 * (∑ i, row i * w i).natAbs : ℕ) : ℝ) := by
      exact_mod_cast hr
    simpa only [Nat.cast_mul, Nat.cast_add, Nat.cast_ofNat,
      Nat.cast_natAbs, Int.cast_abs] using hcast
  have hratio : (10613 : ℝ) * ‖v‖ <
      5550 * |((∑ i, row i * w i : ℤ) : ℝ)| := by
    nlinarith
  let Y : ℝ := Real.sqrt 2 * euclideanRowDot row v / ‖v‖
  have hY : (10613 * Real.sqrt 2 / 5550 : ℝ) < |Y| := by
    dsimp [Y]
  ...
  have hconstant : (10613 * Real.sqrt 2 / 5550 : ℝ) ^ 2 =
      112635769 / 15401250 := by
    rw [div_pow, mul_pow, hsqrt]
    norm_num
  have hsq : (10613 * Real.sqrt 2 / 5550 : ℝ) ^ 2 < |Y| ^ 2 :=
    pow_lt_pow_left₀ hY (by positivity) (by norm_num)
  rw [hconstant, sq_abs] at hsq
  change (36567 / 5000 : ℝ) <
    (Real.sqrt 2 * euclideanRowDot row v / ‖v‖) ^ 2
  exact lt_trans
    (by norm_num : (36567 / 5000 : ℝ) < 112635769 / 15401250) hsq

theorem wrap_row_probability_toReal_lt
    {q d inputThreshold : ℕ} (w : Fin d → ℤ)
    (hpositive : 0 < inputThreshold)
    (hnorm : inputThreshold ^ 2 ≤ sqNorm w) (hs : Small q w) :
    (eventProbability (sparseRademacherRow d)
      (WrapRow q inputThreshold w)).toReal < 367039 / 10000000 := by
  let v := integerEuclideanVector w
  have hVpos : 0 < sqNorm w := (pow_pos hpositive 2).trans_le hnorm
  have hw : w ≠ 0 := (sqNorm_pos_iff w).1 hVpos
  have hv : v ≠ 0 := by
  ...
  have htail := rademacherSum_normalized_tail_36567_over_5000_toReal_lt_fintype
...
theorem small_row_probability_toReal_lt
    ...
    (hnorm : inputThreshold ^ 2 ≤ sqNorm w) (hs : Small q w) :
    (eventProbability (sparseRademacherRow d)
      (RowPass q inputThreshold w)).toReal < 627 / 1000 := by
  ...
  have ht := wrap_row_probability_toReal_lt w hpositive hnorm hs
  calc
    (eventProbability (sparseRademacherRow d)
    ...
    _ < (TernaryLInfTwoDecimal.Cap34Central.expectationBound : ℝ) +
        367039 / 10000000 := add_lt_add_of_le_of_lt hc ht
    _ < 627 / 1000 := by
      norm_num [TernaryLInfTwoDecimal.Cap34Central.expectationBound]
...
theorem large_row_probability_toReal_le_half
  ...
  have hmono : eventProbability sparseEntryPMF
      (fun x => 2500 *
        (centeredMod q (shift + x * w i)).natAbs ^ 2 ≤
          289 * inputThreshold ^ 2) ≤
      eventProbability sparseEntryPMF
        (fun x => ScaledCenteredArcPass q 50 (17 * inputThreshold)
          (shift + x * w i)) := by
    apply eventProbability_mono
    intro x hx
  ...
  apply sparseEntry_scaledCenteredArcPass_toReal_le_half_of_centered
    (q := q) (scale := 50) (radius := 17 * inputThreshold) shift (w i)
...
theorem rows_probability_lt_of_pow_bound
    ...
    (hnorm : inputThreshold ^ 2 ≤ sqNorm w)
    (hmargin : 2 * inputThreshold ≤ q)
    (hpow : ((627 : ENNReal) / 1000) ^ rows < budget) :
  ...
  apply (sparseRademacherMatrix_eventProbability_allRows_le_pow
    rows d (RowPass q inputThreshold w) ((627 : ENNReal) / 1000) ?_).trans_lt
    hpow
...
theorem rowBound_pow_rows192_bits129 :
    ((627 : ENNReal) / 1000) ^ 192 < failureTarget 129 := by
...
theorem ternaryLInfThresholdLower17Over50_of_pow_bound
    (rows bits : ℕ)
    (hcentral : TernaryLInfTwoDecimal.Cap34CentralCertificate)
    (hdiffuse : TernaryLInfTwoDecimal.Cap34DiffuseCertificate)
    (hpow : ((627 : ENNReal) / 1000) ^ rows < failureTarget bits) :
...
theorem ternaryLInfThresholdLower17Over50_rows192_bits129_of_certificates
    (hcentral : TernaryLInfTwoDecimal.Cap34CentralCertificate)
    (hdiffuse : TernaryLInfTwoDecimal.Cap34DiffuseCertificate) :
    LInfThresholdLowerTailAt (parametersAt 192) (failureTarget 129) :=
  ternaryLInfThresholdLower17Over50_of_pow_bound 192 129 hcentral hdiffuse
    rowBound_pow_rows192_bits129

end CertifiedJL.TernaryLInfThresholdLower17Over50TwoDecimal
