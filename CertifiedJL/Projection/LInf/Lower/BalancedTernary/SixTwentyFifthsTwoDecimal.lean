...
/-!
# Sharp two-decimal `6/25` sparse modular infinity lower tail
-/

open scoped BigOperators ENNReal
open MeasureTheory

namespace CertifiedJL.TernaryLInfThresholdLower6Over25TwoDecimal
...
def RowPass (q inputThreshold : ℕ) {d : ℕ} (w : Fin d → ℤ)
    (row : Fin d → ℤ) : Prop :=
  625 * (centeredMod q (∑ i, row i * w i)).natAbs ^ 2 ≤
    36 * inputThreshold ^ 2
...
theorem rowPass_implies_central_or_wrap
  ...
  · left
    ...
    change 625 * z.natAbs ^ 2 ≤ 36 * inputThreshold ^ 2
    change 625 * (centeredMod q z).natAbs ^ 2 ≤
      36 * inputThreshold ^ 2 at hp
    rwa [← hz] at hp
  · right
    have hgeometry := centeredMod_modulus_le_natAbs_add_of_ne hz
    have hr : 25 * (centeredMod q z).natAbs ≤ 6 * inputThreshold := by
      unfold RowPass at hp
      change 625 * (centeredMod q z).natAbs ^ 2 ≤
        36 * inputThreshold ^ 2 at hp
      nlinarith [sq_nonneg
        (25 * ((centeredMod q z).natAbs : ℤ) - 6 * (inputThreshold : ℤ))]
    unfold WrapRow
    change q ≤ z.natAbs + (centeredMod q z).natAbs at hgeometry
    change 25 * q ≤ 6 * inputThreshold + 25 * z.natAbs
    omega

theorem central_row_probability_toReal_le {d inputThreshold : ℕ}
    (hcentral : TernaryLInfTwoDecimal.Cap24CentralCertificate)
    ...
    (eventProbability (sparseRademacherRow d)
      (CentralRow inputThreshold w)).toReal ≤
        (TernaryLInfTwoDecimal.Cap24Central.expectationBound : ℝ) := by
  classical
  ...
  apply central_event_probability_toReal_le w hw (CentralRow inputThreshold w)
    TernaryLInfTwoDecimal.Cap24Central.fourier
  ...
  · rw [if_pos hr]
    ...
    have hrR : (25 : ℝ) * |((∑ i, row i * w i : ℤ) : ℝ)| ≤
        6 * inputThreshold := by
      unfold CentralRow at hr
      have hcast : (625 : ℝ) * |((∑ i, row i * w i : ℤ) : ℝ)| ^ 2 ≤
          36 * (inputThreshold : ℝ) ^ 2 := by
        have hcastNat :
            ((625 * (∑ i, row i * w i).natAbs ^ 2 : ℕ) : ℝ) ≤
              ((36 * inputThreshold ^ 2 : ℕ) : ℝ) := by
          exact_mod_cast hr
    ...
    have hphase : |centralPhase w row| ≤
        (111 / 100 : ℝ) * (6 / 25 : ℝ) * Real.sqrt 2 := by
    ...
    have hcapPi : ((111 / 100 : ℝ) * (6 / 25 : ℝ) * Real.sqrt 2) ≤
        Real.pi := by
    ...
    have hc := cos_central24_lower
    norm_num at hc ⊢
    rw [show (111 / 100 : ℝ) * (6 / 25 : ℝ) = 333 / 1250 by norm_num]
      at hcosMono
...
theorem diffuseArc_row_probability_toReal_lt {q d : ℕ}
    (hdiffuse : TernaryLInfTwoDecimal.Cap24DiffuseCertificate)
  ...
  have hpoint (row : Fin d → ℤ) :
      (if DiffuseArcRow q w row then (1 : ℝ) else 0) ≤
        TrigonometricBernstein.rationalCosineValue
          TernaryLInfTwoDecimal.Cap24Diffuse.fourier
          (diffusePhase (q := q) w row) := by
    by_cases hp : DiffuseArcRow q w row
    · rw [if_pos hp]
      let z : ℤ := ∑ i, row i * w i
      let r : ℤ := centeredMod q z
      have hr : (50 : ℝ) * |(r : ℝ)| ≤ 6 * q := by
        have hc : 50 * r.natAbs ≤ 6 * q := hp
        have hc' : ((50 * r.natAbs : ℕ) : ℝ) ≤ ((6 * q : ℕ) : ℝ) := by
          exact_mod_cast hc
      ...
      have hcapPi : (6 * Real.pi / 25 : ℝ) ≤ Real.pi := by
        nlinarith [Real.pi_pos]
      have hcosMono : Real.cos (6 * Real.pi / 25) ≤
          Real.cos ((2 * Real.pi / (q : ℝ)) * (r : ℝ)) := by
        calc
      ...
      rw [hcosPhase]
      have hc := cos_diffuse24_lower
      norm_num at hc ⊢
      rw [show 6 * Real.pi / 25 = Real.pi * (6 / 25 : ℝ) by ring]
        at hcosMono
      exact hc.le.trans hcosMono
    · rw [if_neg hp]
  ...
  have hmomentOne :
      (∫ seed, Real.cos ((1 : ℝ) *
        diffusePhase (q := q) w (sparseRow seed)) ∂p.toMeasure) ≤
          (TernaryLInfTwoDecimal.Cap24Diffuse.diffuseMomentBound : ℝ) := by
    ...
    have hsum : Real.pi ^ 2 * (113 / 250 : ℝ) ^ 2 ≤
        ∑ i, (Real.pi * (w i : ℝ) / (q : ℝ)) ^ 2 := by
      have hdR : (113 : ℝ) ^ 2 * q ^ 2 ≤
    ...
    have hstrict : (∏ i, Real.cos
        (Real.pi * (w i : ℝ) / (q : ℝ)) ^ 2) < 333 / 2500 := calc
      ...
      _ ≤ Real.exp (-(Real.pi ^ 2 * (113 / 250 : ℝ) ^ 2)) :=
        Real.exp_le_exp.mpr (by linarith)
      _ < 333 / 2500 := exp_neg_diffuse24_lt
    norm_num [TernaryLInfTwoDecimal.Cap24Diffuse.diffuseMomentBound]
      at hstrict ⊢
    exact hstrict.le
  ...
  exact hbound.trans_lt (by
    norm_num [TernaryLInfTwoDecimal.Cap24Diffuse.expectationBound])

theorem wrap_implies_tail_389_over_50_lt
  ...
  have hsR : (250 : ℝ) ^ 2 * (sqNorm w : ℝ) <
      (113 : ℝ) ^ 2 * q ^ 2 := by
    exact_mod_cast hs
  have hqNorm : (250 : ℝ) * ‖v‖ < 113 * q := by
    rw [← hnormSq] at hsR
    nlinarith [norm_nonneg v]
  have hrR : (25 : ℝ) * q ≤ 6 * inputThreshold +
      25 * |((∑ i, row i * w i : ℤ) : ℝ)| := by
    unfold WrapRow at hr
    have hcast : ((25 * q : ℕ) : ℝ) ≤
        ((6 * inputThreshold + 25 * (∑ i, row i * w i).natAbs : ℕ) : ℝ) := by
      exact_mod_cast hr
    simpa only [Nat.cast_mul, Nat.cast_add, Nat.cast_ofNat,
      Nat.cast_natAbs, Int.cast_abs] using hcast
  have hratio : (5572 : ℝ) * ‖v‖ <
      2825 * |((∑ i, row i * w i : ℤ) : ℝ)| := by
    nlinarith
  let Y : ℝ := Real.sqrt 2 * euclideanRowDot row v / ‖v‖
  have hY : (5572 * Real.sqrt 2 / 2825 : ℝ) < |Y| := by
    dsimp [Y]
  ...
  have hconstant : (5572 * Real.sqrt 2 / 2825 : ℝ) ^ 2 =
      62094368 / 7980625 := by
    rw [div_pow, mul_pow, hsqrt]
    norm_num
  have hsq : (5572 * Real.sqrt 2 / 2825 : ℝ) ^ 2 < |Y| ^ 2 :=
    pow_lt_pow_left₀ hY (by positivity) (by norm_num)
  rw [hconstant, sq_abs] at hsq
  change (389 / 50 : ℝ) <
    (Real.sqrt 2 * euclideanRowDot row v / ‖v‖) ^ 2
  exact lt_trans (by norm_num : (389 / 50 : ℝ) < 62094368 / 7980625) hsq
...
theorem wrap_row_probability_toReal_lt
    ...
    (hnorm : inputThreshold ^ 2 ≤ sqNorm w) (hs : Small q w) :
    (eventProbability (sparseRademacherRow d)
      (WrapRow q inputThreshold w)).toReal < 287 / 10000 := by
  let v := integerEuclideanVector w
  have hVpos : 0 < sqNorm w := (pow_pos hpositive 2).trans_le hnorm
  have hw : w ≠ 0 := (sqNorm_pos_iff w).1 hVpos
  have hv : v ≠ 0 := by
  ...
  have hmono : eventProbability (sparseRademacherRow d)
    ...
    eventProbability_mono (sparseRademacherRow d)
      (fun row (hr : WrapRow q inputThreshold w row) =>
        (wrap_implies_tail_389_over_50_lt hpositive hnorm hs hr).le)
  ...
  have htail := rademacherSum_normalized_tail_389_over_50_toReal_lt_fintype
...
theorem small_row_probability_toReal_lt
    ...
    (hnorm : inputThreshold ^ 2 ≤ sqNorm w) (hs : Small q w) :
    (eventProbability (sparseRademacherRow d)
      (RowPass q inputThreshold w)).toReal < 2933 / 5000 := by
  ...
  calc
    ...
    _ < (TernaryLInfTwoDecimal.Cap24Central.expectationBound : ℝ) +
        287 / 10000 := add_lt_add_of_le_of_lt hc ht
    _ < 2933 / 5000 := by
      norm_num [TernaryLInfTwoDecimal.Cap24Central.expectationBound]
...
theorem diffuse_row_probability_toReal_lt
  ...
  have hmono : eventProbability (sparseRademacherRow d)
    ...
    nlinarith [sq_nonneg
      (25 * ((centeredMod q (∑ i, row i * w i)).natAbs : ℤ) -
        6 * (inputThreshold : ℤ))]
...
theorem large_row_probability_toReal_le_half
  ...
  have hmono : eventProbability sparseEntryPMF
      (fun x => 625 *
        (centeredMod q (shift + x * w i)).natAbs ^ 2 ≤
          36 * inputThreshold ^ 2) ≤
      eventProbability sparseEntryPMF
        (fun x => ScaledCenteredArcPass q 25 (6 * inputThreshold)
          (shift + x * w i)) := by
    apply eventProbability_mono
    intro x hx
    unfold ScaledCenteredArcPass
    nlinarith [sq_nonneg
      (25 * ((centeredMod q (shift + x * w i)).natAbs : ℤ) -
        6 * (inputThreshold : ℤ))]
  ...
  apply sparseEntry_scaledCenteredArcPass_toReal_le_half_of_centered
    (q := q) (scale := 25) (radius := 6 * inputThreshold) shift (w i)
...
theorem rows_probability_lt_of_pow_bound
    ...
    (hnorm : inputThreshold ^ 2 ≤ sqNorm w)
    (hmargin : 2 * inputThreshold ≤ q)
    (hpow : ((2933 : ENNReal) / 5000) ^ rows < budget) :
  ...
  apply (sparseRademacherMatrix_eventProbability_allRows_le_pow
    rows d (RowPass q inputThreshold w) ((2933 : ENNReal) / 5000) ?_).trans_lt
    hpow
...
theorem rowBound_pow_rows256_bits197 :
    ((2933 : ENNReal) / 5000) ^ 256 < failureTarget 197 := by
...
theorem ternaryLInfThresholdLower6Over25_of_pow_bound
    (rows bits : ℕ)
    (hcentral : TernaryLInfTwoDecimal.Cap24CentralCertificate)
    (hdiffuse : TernaryLInfTwoDecimal.Cap24DiffuseCertificate)
    (hpow : ((2933 : ENNReal) / 5000) ^ rows < failureTarget bits) :
...
theorem ternaryLInfThresholdLower6Over25_rows256_bits197_of_certificates
    (hcentral : TernaryLInfTwoDecimal.Cap24CentralCertificate)
    (hdiffuse : TernaryLInfTwoDecimal.Cap24DiffuseCertificate) :
    LInfThresholdLowerTailAt (parametersAt 256) (failureTarget 197) :=
  ternaryLInfThresholdLower6Over25_of_pow_bound 256 197 hcentral hdiffuse
    rowBound_pow_rows256_bits197

end CertifiedJL.TernaryLInfThresholdLower6Over25TwoDecimal
