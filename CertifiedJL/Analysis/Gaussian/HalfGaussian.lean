...
/-! Compatibility API for the original 256-dimensional half-Gaussian specialization. -/
...
noncomputable def halfGaussianRadialDensity :
    EuclideanSpace ℝ (Fin 256) → ℝ :=
  fun x => Real.pi⁻¹ ^ 128 * Real.exp (-‖x‖ ^ 2)
noncomputable def halfGaussianProductDensity : (Fin 256 → ℝ) → ℝ :=
  fun x => ∏ i, halfGaussianDensity (x i)
def squaredRadius : (Fin 256 → ℝ) → ℝ := fun x => ∑ i, (x i) ^ 2
...
theorem integrable_halfGaussianProductDensity : Integrable halfGaussianProductDensity := by
  exact Integrable.fintype_prod
    (μ := fun _ : Fin 256 => (volume : Measure ℝ))
    (fun _ => integrable_halfGaussianDensity)
theorem halfGaussianProductDensity_eq_radial (x : Fin 256 → ℝ) :
  ...
  simpa [halfGaussianProductDensity, halfGaussianDensity, halfGaussianRadialDensity,
    ...
    HalfGaussianEven.halfGaussianRadialDensity] using
      HalfGaussianEven.halfGaussianProductDensity_eq_radial
        (rows := 256) (shape := 128) (by norm_num) x
theorem halfGaussianProductDensity_eq_exp (x : Fin 256 → ℝ) :
    halfGaussianProductDensity x = Real.pi⁻¹ ^ 128 * Real.exp (-squaredRadius x) := by
  simpa [halfGaussianProductDensity, halfGaussianDensity, squaredRadius,
    ...
    HalfGaussianEven.squaredRadius] using
      HalfGaussianEven.halfGaussianProductDensity_eq_exp
        (rows := 256) (shape := 128) (by norm_num) x
theorem integral_pow_255_exp_neg_sq_Ioi {a : ℝ} (ha : 0 ≤ a) :
    (∫ r : ℝ in Set.Ioi (Real.sqrt a), r ^ 255 * Real.exp (-r ^ 2)) =
      (1 / 2 : ℝ) * ∫ t : ℝ in Set.Ioi a, t ^ 127 * Real.exp (-t) := by
...
noncomputable def tiltedHalfGaussianProductDensity : (Fin 256 → ℝ) → ℝ :=
...
theorem integrable_tiltedHalfGaussianProductDensity :
  ...
  exact Integrable.fintype_prod (μ := fun _ : Fin 256 => (volume : Measure ℝ))
    (fun _ => integrable_tiltedHalfGaussianDensity)
theorem tiltedHalfGaussianProductDensity_eq_exp (x : Fin 256 → ℝ) :
    tiltedHalfGaussianProductDensity x = Real.pi⁻¹ ^ 128 *
  ...
  simpa [tiltedHalfGaussianProductDensity, tiltedHalfGaussianDensity,
    ...
    HalfGaussianEven.tiltedHalfGaussianDensity] using
      HalfGaussianEven.tiltedHalfGaussianProductDensity_eq_exp
        (rows := 256) (shape := 128) (by norm_num) x
theorem halfGaussianProductDensity_eq_exp_mul_tilted (x : Fin 256 → ℝ) :
...
theorem coordinateTiltedTailDensity_nonneg
    (b : ℝ) (j : Fin 256) (x : Fin 256 → ℝ) :
    0 ≤ coordinateTiltedTailDensity b j x := by
  exact HalfGaussianEven.coordinateTiltedTailDensity_nonneg (rows := 256) b j x
theorem coordinateTiltedTailDensity_eq_of_mem
    {b : ℝ} {j : Fin 256} {x : Fin 256 → ℝ} (hj : b ≤ |x j|) :
    coordinateTiltedTailDensity b j x = tiltedHalfGaussianProductDensity x := by
  exact HalfGaussianEven.coordinateTiltedTailDensity_eq_of_mem (rows := 256) hj
theorem integrable_coordinateTiltedTailDensity (b : ℝ) (j : Fin 256) :
    Integrable (coordinateTiltedTailDensity b j) := by
  exact HalfGaussianEven.integrable_coordinateTiltedTailDensity (rows := 256) b j
theorem integral_coordinateTiltedTailDensity (b : ℝ) (j : Fin 256) :
    (∫ x : Fin 256 → ℝ, coordinateTiltedTailDensity b j x) =
      (∫ t : ℝ in {t | b ≤ |t|}, tiltedHalfGaussianDensity t) *
        Real.sqrt (5 / 2 : ℝ) ^ 255 := by
  exact HalfGaussianEven.integral_coordinateTiltedTailDensity (rows := 256) b j

def halfGaussianTruncationSet : ℝ → ℝ → Set (Fin 256 → ℝ) :=
...
theorem measurableSet_halfGaussianTruncationSet (a b : ℝ) :
  ...
  simpa [halfGaussianTruncationSet, squaredRadius,
    ...
    HalfGaussianEven.squaredRadius] using
      HalfGaussianEven.measurableSet_halfGaussianTruncationSet (rows := 256) a b
theorem integral_halfGaussianTruncationSet_lt {a b : ℝ} (hb : 0 < b)
    (hscale : (Real.sqrt Real.pi)⁻¹ / ((2 / 5 : ℝ) * b) < Real.sqrt (5 / 2 : ℝ)) :
    (∫ x : Fin 256 → ℝ in halfGaussianTruncationSet a b,
        halfGaussianProductDensity x) <
      256 * (5 / 2 : ℝ) ^ 128 * Real.exp (-(3 * a + 2 * b ^ 2) / 5) := by
  simpa [halfGaussianTruncationSet, squaredRadius, halfGaussianProductDensity,
    ...
    (rows := 256) (shape := 128) (by norm_num) (by norm_num)
    (a := a) (b := b) hb hscale
theorem integral_halfGaussianRadialDensity_tail {a : ℝ} (ha : 0 ≤ a) :
    (∫ x : EuclideanSpace ℝ (Fin 256) in {x | a < ‖x‖ ^ 2},
        halfGaussianRadialDensity x) = gammaSurvivalNat 128 a := by
...
theorem integral_halfGaussianProductDensity_tail {a : ℝ} (ha : 0 ≤ a) :
    (∫ x : Fin 256 → ℝ in {x | a < ∑ i, (x i) ^ 2},
        halfGaussianProductDensity x) = gammaSurvivalNat 128 a := by
