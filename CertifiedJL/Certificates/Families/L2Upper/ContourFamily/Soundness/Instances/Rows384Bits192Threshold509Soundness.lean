...
/-!
# Soundness facts for the 384-row, 192-bit, threshold-509 instance
...
namespace CertifiedJL.SparseUpperContourFamily.Instances.Rows384Bits192Threshold509Soundness
...
theorem lowProfileFacts : ∀ box ∈ profileBoxes,
  ...
  all_goals exact ⟨192, by norm_num⟩
...
theorem highProfileFacts : HighProfileFacts parameters highProfile where
  ...
  rows_even := ⟨192, by norm_num [parameters, Parameters.rows]⟩
  capContains := by
...
def threshold : NonnegativeRatio := NonnegativeRatio.ofNat 509
...
end CertifiedJL.SparseUpperContourFamily.Instances.Rows384Bits192Threshold509Soundness
