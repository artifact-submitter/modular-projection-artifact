...
/-!
# Soundness facts for the 512-row, 256-bit, threshold-681 instance
...
namespace CertifiedJL.SparseUpperContourFamily.Instances.Rows512Bits256Threshold681Soundness
...
theorem lowProfileFacts : ∀ box ∈ profileBoxes,
  ...
  all_goals exact ⟨256, by norm_num⟩
...
theorem highProfileFacts : HighProfileFacts parameters highProfile where
  ...
  rows_even := ⟨256, by norm_num [parameters, Parameters.rows]⟩
  capContains := by
...
def threshold : NonnegativeRatio := NonnegativeRatio.ofNat 681
...
end CertifiedJL.SparseUpperContourFamily.Instances.Rows512Bits256Threshold681Soundness
