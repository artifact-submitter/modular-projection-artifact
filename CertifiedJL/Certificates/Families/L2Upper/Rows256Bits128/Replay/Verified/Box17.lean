import CertifiedJL.Certificates.Families.L2Upper.Rows256Bits128.Replay.Shards.Box17.Shard00

namespace CertifiedJL.SparseUpperHybrid

open UpperContourKernel

private def certificate : CertificateBox := certificateBoxes.getD 17 default

private def generatedCertificate : CertificateBox :=
  ⟨⟨1 / 32, 1 / 20, 631 / 1000, 6, 6 / 5⟩,
    [ ⟨0, 1 / 19, 19⟩,
      ⟨1, 1 / 4, 4⟩,
      ⟨2, 1, 2⟩,
      ⟨4, 1, 4⟩,
...
private theorem computedChunks_eq_generated :
    (certificateChunkPlan certificate).map (certificateChunkValue certificate) =
      SparseUpperHybridGenerated.Box17.chunks := by
  ...
  rw [Internal.Box17.chunk_000,
    Internal.Box17.chunk_001,
    Internal.Box17.chunk_002,
    Internal.Box17.chunk_003,
    Internal.Box17.chunk_004]
  norm_num [SparseUpperHybridGenerated.Box17.chunk,
...
private theorem segment0_chunkRanges_cover :
    chunkRangesCover 19 segment0_chunkRanges = true := by
  decide +kernel

private theorem segment0_chunkRanges_sideChecks :
    segment0_chunkRanges.all (fun chunk =>
      segmentChunkSideCheck ⟨1 / 32, 1 / 20, 631 / 1000, 6, 6 / 5⟩ ⟨0, 1 / 19, 19⟩
        chunk.1 chunk.2) = true := by
  simp only [segment0_chunkRanges, List.all_cons, List.all_nil, Bool.and_eq_true]
  exact ⟨Internal.Box17.chunk_000_sideCheck,
    Internal.Box17.chunk_001_sideCheck,
    True.intro⟩
...
private theorem segment2_chunkRanges_sideChecks :
    segment2_chunkRanges.all (fun chunk =>
      segmentChunkSideCheck ⟨1 / 32, 1 / 20, 631 / 1000, 6, 6 / 5⟩ ⟨2, 1, 2⟩
        chunk.1 chunk.2) = true := by
  simp only [segment2_chunkRanges, List.all_cons, List.all_nil, Bool.and_eq_true]
  exact ⟨Internal.Box17.chunk_003_sideCheck,
    True.intro⟩
...
private theorem segment3_chunkRanges_sideChecks :
    segment3_chunkRanges.all (fun chunk =>
      segmentChunkSideCheck ⟨1 / 32, 1 / 20, 631 / 1000, 6, 6 / 5⟩ ⟨4, 1, 4⟩
        chunk.1 chunk.2) = true := by
  simp only [segment3_chunkRanges, List.all_cons, List.all_nil, Bool.and_eq_true]
  exact ⟨Internal.Box17.chunk_004_sideCheck,
    True.intro⟩
...
private theorem segment4_chunkRanges_sideChecks :
    segment4_chunkRanges.all (fun chunk =>
      segmentChunkSideCheck ⟨1 / 32, 1 / 20, 631 / 1000, 6, 6 / 5⟩ ⟨8, 1, 0⟩
        chunk.1 chunk.2) = true := by rfl


theorem certificate_17_finiteTail_lo_eq_zero :
    (finiteIntegralFrom
      ((certificateChunkPlan (certificateBoxes.getD 17 default)).map
        (certificateChunkValue (certificateBoxes.getD 17 default))) +
      tail (certificateBoxes.getD 17 default).box).lo = 0 := by
...
theorem certificate_17_finiteTail_nonneg :
    0 ≤ (finiteIntegralFrom
      ((certificateChunkPlan (certificateBoxes.getD 17 default)).map
        (certificateChunkValue (certificateBoxes.getD 17 default))) +
      tail (certificateBoxes.getD 17 default).box).lo := by
  rw [certificate_17_finiteTail_lo_eq_zero]

theorem certificate_17_finiteTail_valid :
    (finiteIntegralFrom
      ((certificateChunkPlan (certificateBoxes.getD 17 default)).map
        (certificateChunkValue (certificateBoxes.getD 17 default))) +
      tail (certificateBoxes.getD 17 default).box).Valid := by
  change (finiteIntegralFrom
    ((certificateChunkPlan certificate).map (certificateChunkValue certificate)) +
      tail certificate.box).Valid
  rw [computedChunks_eq_generated, certificate_eq_generated]
  change (finiteIntegralFrom SparseUpperHybridGenerated.Box17.chunks +
      tail generatedCertificate.box).lo ≤
    (finiteIntegralFrom SparseUpperHybridGenerated.Box17.chunks +
      tail generatedCertificate.box).hi
  decide +kernel

set_option maxRecDepth 10000 in
theorem certificate_17_sideChecks :
    ∀ segment ∈ (certificateBoxes.getD 17 default).segments,
      ∀ i < segment.count,
        hybridCellSideCheck (certificateBoxes.getD 17 default).box
          segment i = true := by
...
private def generatedBound : DInterval precision :=
  boxBoundFrom certificate.box SparseUpperHybridGenerated.Box17.chunks

private theorem generatedCheck :
    Interval.upperLTCheck generatedBound 1 = true := by
  decide +kernel

theorem certificate_17_upperRat_lt :
    (certificateBound (certificateBoxes.getD 17 default)).upperRat < 1 := by
