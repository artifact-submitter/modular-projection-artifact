...
namespace CertifiedJL.SparseThresholdDominant.CappedFourierNumeric256Bits192
...
def twoPiDivUpper (cell : Cell) : DInterval :=
  rat 2 * piDivUpper cell
...
def secondResidualMode (cell : Cell) : DInterval :=
  expUpper (-(rat (7 / 200) * powNat (twoPiDivUpper cell) 2 * rat (1 / 2)))
...
def rowInterval (cell : Cell) : DInterval :=
  ...
  let modeTwo := expUpper (-(rat 4 * c)) *
    (reflectedPhase cell * secondResidualMode cell)
...
def safeCheck (cell : Cell) : Bool :=
  decide (
    ...
    (-(rat (7 / 200) * powNat (twoPiDivUpper cell) 2 *
      rat (1 / 2))).upperRat ≤ 1 ∧
...
end CertifiedJL.SparseThresholdDominant.CappedFourierNumeric256Bits192
