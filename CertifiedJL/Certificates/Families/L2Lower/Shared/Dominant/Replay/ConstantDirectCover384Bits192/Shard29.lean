...
private theorem shard29_row_caps :
    ConstantDirectCover128Shard29.entries.all (fun entry =>
      rowCapsCheck entry.cell (targetCertificate entry)) = true := by
  exact all_rowCapsCheck_of_eq
    ConstantDirectCover128Shard29.entries (fun entry => entry.cell) targetCertificate
    (by decide +kernel) (by decide +kernel)
    ConstantDirectCover128Shard29.entries_row_caps

set_option maxRecDepth 100000 in
private theorem shard29_target_certified :
    ConstantDirectCover128Shard29.entries.all (fun entry =>
...
theorem shard29_certified :
    ConstantDirectCover128Shard29.entries.all targetCheck = true := by
  simpa [targetCheck] using
    all_localCertifiedCheckFor_of_rowCaps 384 43
      ConstantDirectCover128Shard29.entries (fun entry => entry.cell)
      targetCertificate budget shard29_row_caps
      shard29_target_certified
