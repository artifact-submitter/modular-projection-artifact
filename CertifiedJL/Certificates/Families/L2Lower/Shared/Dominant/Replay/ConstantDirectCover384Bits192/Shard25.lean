...
private theorem shard25_row_caps :
    ConstantDirectCover128Shard25.entries.all (fun entry =>
      rowCapsCheck entry.cell (targetCertificate entry)) = true := by
  exact all_rowCapsCheck_of_eq
    ConstantDirectCover128Shard25.entries (fun entry => entry.cell) targetCertificate
    (by decide +kernel) (by decide +kernel)
    ConstantDirectCover128Shard25.entries_row_caps

set_option maxRecDepth 100000 in
private theorem shard25_target_certified :
    ConstantDirectCover128Shard25.entries.all (fun entry =>
      localTargetCheckFor 384 43 ((fun entry => entry.cell) entry)
        (targetCertificate entry) budget) = true := by
  decide +kernel

theorem shard25_certified :
    ConstantDirectCover128Shard25.entries.all targetCheck = true := by
  simpa [targetCheck] using
    all_localCertifiedCheckFor_of_rowCaps 384 43
      ConstantDirectCover128Shard25.entries (fun entry => entry.cell)
      targetCertificate budget shard25_row_caps
      shard25_target_certified
