...
/-! # Replayed capped-Fourier cover for 512 rows at 192 bits -/

namespace CertifiedJL.SparseThresholdDominant.CappedFourierCover512Bits192
...
theorem cells_certified :
    cells.all (certifiedCheckAt 512 71 budget) = true := by
  change cells.all (fun cell => safeCheck cell &&
    TargetNumeric.certifiedCheckAt 512 71 z
...
theorem certified_of_mem {cell : Cell} (hmem : cell ∈ cells) :
    certifiedCheckAt 512 71 budget cell = true := by
...
theorem exists_cover_cell {B r : ℝ}
    ...
    ∃ cell : Cell, cell ∈ cells ∧
      ...
      Real.exp (71 * (z : ℝ) * (cell.thresholdUpper : ℝ)) *
  ...
  have hgeometry := certifiedCheckAt_geometry 512 71 budget cell hcert
  exact ⟨cell, hmem, hlower, hupper, hthreshold, hgeometry.1,
    hgeometry.2, certifiedCheckAt_sound 512 71 budget cell hcert⟩

end CertifiedJL.SparseThresholdDominant.CappedFourierCover512Bits192
