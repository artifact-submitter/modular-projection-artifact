...
private theorem shard12_row_caps :
    ConstantDirectCover128Shard12.entries.all (fun entry =>
      rowCapsCheck entry.cell (targetCertificate entry)) = true := by
  exact all_rowCapsCheck_of_eq
    ConstantDirectCover128Shard12.entries (fun entry => entry.cell) targetCertificate
    (by decide +kernel) (by decide +kernel)
    ConstantDirectCover128Shard12.entries_row_caps

set_option maxRecDepth 100000 in
private theorem shard12_target_certified :
    ConstantDirectCover128Shard12.entries.all (fun entry =>
...
theorem shard12_certified :
    ConstantDirectCover128Shard12.entries.all targetCheck = true := by
  simpa [targetCheck] using
    all_localCertifiedCheckFor_of_rowCaps 256 9
      ConstantDirectCover128Shard12.entries (fun entry => entry.cell)
      targetCertificate budget shard12_row_caps
      shard12_target_certified
