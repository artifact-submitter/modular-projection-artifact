...
private theorem shard04_row_caps :
    ConstantDirectCover128Shard04.entries.all (fun entry =>
      rowCapsCheck entry.cell (targetCertificate entry)) = true := by
  exact all_rowCapsCheck_of_eq
    ConstantDirectCover128Shard04.entries (fun entry => entry.cell) targetCertificate
    (by decide +kernel) (by decide +kernel)
    ConstantDirectCover128Shard04.entries_row_caps

set_option maxRecDepth 100000 in
private theorem shard04_target_certified :
    ConstantDirectCover128Shard04.entries.all (fun entry =>
      localTargetCheckFor 256 9 ((fun entry => entry.cell) entry)
        (targetCertificate entry) budget) = true := by
  decide +kernel

theorem shard04_certified :
    ConstantDirectCover128Shard04.entries.all targetCheck = true := by
  simpa [targetCheck] using
    all_localCertifiedCheckFor_of_rowCaps 256 9
      ConstantDirectCover128Shard04.entries (fun entry => entry.cell)
      targetCertificate budget shard04_row_caps
      shard04_target_certified
