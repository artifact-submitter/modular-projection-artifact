...
private theorem shard42_row_caps :
  ...
  exact all_rowCapsCheck_of_eq
    ConstantDirectCover128Shard42.entries (fun entry => (retargetEntry entry).cell)
...
theorem shard42_certified :
  ...
  simpa [targetCheck] using
    all_localCertifiedCheckFor_of_rowCaps 192 12
      ConstantDirectCover128Shard42.entries (fun entry => (retargetEntry entry).cell)
      (fun entry => (retargetEntry entry).certificate) budget shard42_row_caps
