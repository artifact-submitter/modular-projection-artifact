...
private theorem shard24_row_caps :
  ...
  exact all_rowCapsCheck_of_eq
    ConstantDirectCover128Shard24.entries (fun entry => (retargetEntry entry).cell)
    (fun entry => (retargetEntry entry).certificate)
    (by decide +kernel) (by decide +kernel)
    ConstantDirectCover128Shard24.entries_row_caps
...
theorem shard24_certified :
  ...
  simpa [targetCheck] using
    all_localCertifiedCheckFor_of_rowCaps 192 12
      ConstantDirectCover128Shard24.entries (fun entry => (retargetEntry entry).cell)
      (fun entry => (retargetEntry entry).certificate) budget shard24_row_caps
