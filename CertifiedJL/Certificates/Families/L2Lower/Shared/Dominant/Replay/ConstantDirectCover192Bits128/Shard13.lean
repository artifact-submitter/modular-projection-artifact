...
private theorem shard13_row_caps :
  ...
  exact all_rowCapsCheck_of_eq
    ConstantDirectCover128Shard13.entries (fun entry => (retargetEntry entry).cell)
    (fun entry => (retargetEntry entry).certificate)
    (by decide +kernel) (by decide +kernel)
    ConstantDirectCover128Shard13.entries_row_caps
...
theorem shard13_certified :
  ...
  simpa [targetCheck] using
    all_localCertifiedCheckFor_of_rowCaps 192 12
      ConstantDirectCover128Shard13.entries (fun entry => (retargetEntry entry).cell)
      (fun entry => (retargetEntry entry).certificate) budget shard13_row_caps
