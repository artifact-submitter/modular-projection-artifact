...
private theorem shard02_row_caps :
  ...
  exact all_rowCapsCheck_of_eq
    ConstantDirectCover128Shard02.entries (fun entry => (retargetEntry entry).cell)
    (fun entry => (retargetEntry entry).certificate)
    (by decide +kernel) (by decide +kernel)
    ConstantDirectCover128Shard02.entries_row_caps
...
theorem shard02_certified :
  ...
  simpa [targetCheck] using
    all_localCertifiedCheckFor_of_rowCaps 192 12
      ConstantDirectCover128Shard02.entries (fun entry => (retargetEntry entry).cell)
      (fun entry => (retargetEntry entry).certificate) budget shard02_row_caps
