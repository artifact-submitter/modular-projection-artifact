...
import CertifiedJL.Certificates.Families.L2Lower.Shared.Dominant.Data.SingletonPhaseFourierNumeric256Bits192Data
...
/-!
# Target-specific phase-retaining singleton Fourier cells for the 256-row, 192-bit endpoint
...
namespace SingletonPhaseFourierNumeric256Bits192
...
private theorem rowInterval_contains (cell : Cell)
  ...
  have hmodeTwo := Interval.contains_mul hexpFour hphase
  ...
  simpa [rowInterval, cUpper, piDivUpper, firstMode,
    prefactor, thresholdDominantSingletonPhaseFourierCellRow,
    z] using hrow

/-- The target-selected phase-retaining singleton interval supplies a
reusable nonnegative semantic row cap. -/
theorem semanticRow_bounds_of_safeCheck (cell : Cell)
    (hcheck : safeCheck cell = true) :
    0 ≤ thresholdDominantSingletonPhaseFourierCellRow
  ...
  rcases h with ⟨hlower, hlowerUpper, hdomainUpper, hthreshold,
    ...
    hnineSafe, hfirstSafe, htailDenPos, hgrowthSafe, hrowUpper⟩
  have hrowContains := rowInterval_contains cell hcheck
  have hupper : thresholdDominantSingletonPhaseFourierCellRow
  ...
  refine ⟨?_, hupper⟩
  unfold thresholdDominantSingletonPhaseFourierCellRow
  dsimp only
  positivity
...
theorem certifiedCheck_sound (cell : Cell)
  ...
  have hrow : thresholdDominantSingletonPhaseFourierCellRow
      (cell.lower : ℝ) (cell.upper : ℝ) z ≤ (cell.rowUpper : ℝ) :=
    hrowContains.2.trans (by
      have hreal : ((rowInterval cell).upperRat : ℝ) ≤
          (cell.rowUpper : ℝ) := by exact_mod_cast hrowUpper
  ...
  have hrowNonnegative : 0 ≤ thresholdDominantSingletonPhaseFourierCellRow
      (cell.lower : ℝ) (cell.upper : ℝ) z := by
    have hLreal : (0 : ℝ) < cell.lower := by
      exact_mod_cast (lt_of_lt_of_le (by norm_num : (0 : ℚ) < 2) hlower)
...
end SingletonPhaseFourierNumeric256Bits192
